...
export async function POST() {
  try {
    ...
    const adminToken = (await cookieStore).get('adminToken');

    // If the adminToken doesn't exist, return a 401 error
    if (!adminToken) {
    ...
    // Delete the adminToken cookie (set maxAge to 0 to expire it)
