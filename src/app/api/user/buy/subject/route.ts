    ...
    try {
        ...
        if (existingRows.length > 0) {
            ...
            if (task_type == "update") {
                ...
                await db.query(
                    'UPDATE sold_subjects SET purchase_date = ?, validity = ?, update_date = ?, activeStatus = ?  WHERE id = ? and subject_id = ? ',
                    [purchase_date, validity, purchase_date, 1, ExistingID, subject_id]
                );
